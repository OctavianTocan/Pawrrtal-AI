...
def _replace_match(match: re.Match[str]) -> str:
    ...
    Walks the alternation's capture groups and keeps the first non-None
    one as the prefix. Falls back to a bare ``***`` when no group
    captured (shouldn't happen with the current set, but the explicit
    fallback keeps the function total).
    """
    for group in match.groups():
        if group is not None:
            return group + _REDACTED_SUFFIX
