...
- **Nesting-depth budget**: max 3 levels of compound-statement nesting per Python function (`if`/`for`/`while`/`try`/`with`/`match`). `python3 scripts/check-nesting.py` enforces it on every backend pytest CI run. Use guard clauses and helper functions to flatten — see `.claude/rules/clean-code/limit-nesting-depth.md`. Pre-existing offenders are tracked in the script's `EXEMPT_FUNCTIONS`; do not add new entries.
