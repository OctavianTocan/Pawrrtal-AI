...
- `.github/workflows/check.yml` runs `bun run check` (Biome + tsc + file-lines) on every PR/push to development/main, on the `[self-hosted, ainexus]` runner. Posts a PR comment on failure.
...
- ai-nexus-gvsb (high, bug) — backend pytest collection error: `tests/test_providers_and_schemas.py` imports a renamed/removed symbol `GeminiLLM`.
- ai-nexus-xzix (high, bug) — frontend Vitest has 3 failing tests in `GeneralSection.test.tsx` against stale rendered strings.
- Existing ai-nexus-8nw3 (high, bug) — `NavChatsView.tsx` already over the 500-line budget; `check.yml` will catch it on first run.
...
- [x] Use the `[self-hosted, ainexus]` runner labels
