...
Frontend rip executes in the same commit as this doc. Backend appearance routes (`/api/v1/appearance`) are scheduled for removal in a follow-up bean.
</Callout>
...
```css title="frontend/app/globals.css"
:root {
