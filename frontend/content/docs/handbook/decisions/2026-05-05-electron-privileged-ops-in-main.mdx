...
Privileged ops live in the Electron main process. Agent stays in FastAPI. Desktop-only tools use the client-tool pattern. Date: 2026-05-05. Related: branch `feat/electron-privileged-ops`, beans `#pawrrtal-5rwv`, `#pawrrtal-e7mp`.
</Callout>
...
<Callout type="warn" title="PyInstaller is NOT the chosen strategy">
PyInstaller bundling was rejected for v1 due to known fragility with native deps (`numpy`, `tiktoken`, `cryptography`, `sqlalchemy`), 80–150 MB download overhead, subprocess lifecycle risks, and added latency. It is preserved as a fallback bean (`#pawrrtal-om7i`) should the two-implementation tax ever become load-bearing.
</Callout>
