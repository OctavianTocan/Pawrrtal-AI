...
`@octavian-tocan/react-chat-composer` ships styles as a Tailwind v4 preset with a `chat-*` namespaced token theme. Consumers opt in via one `@import` + `@source` directive.
</Callout>
