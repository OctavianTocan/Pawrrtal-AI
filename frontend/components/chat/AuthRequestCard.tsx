...
import { Spinner } from '@/components/craft-ui'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import { cn } from '@/lib/utils'
import type { Message, CredentialResponse } from '@/lib/craft-shared/types'
import type { AuthRequestType, AuthStatus } from '@/lib/craft-core-stubs'
...
export function AuthRequestCard({ message, onRespondToCredential, sessionId, isInteractive = true }: AuthRequestCardProps) {
  ...
  const isValid = isBasicAuth
    ...
    : isMultiHeader
    ? authHeaderNames?.every((name: string) => headerValues[name]?.trim().length > 0) ?? false
    : value.trim().length > 0
    ...
    return (
      <div className="space-y-3">
        {isBasicAuth ? (
          ...
          <>
            {authHeaderNames.map((headerName: string, index: number) => (
