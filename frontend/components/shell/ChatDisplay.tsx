...
} from "@/components/craft-ui"
import { useFocusZone } from "@/hooks/keyboard"
import { useTheme } from "@/hooks/useTheme"
import type { Session, Message, FileAttachment, StoredAttachment, PermissionRequest, CredentialRequest, CredentialResponse, LoadedSource, LoadedSkill } from '@/lib/craft-shared/types'
import type { PermissionMode } from '@/lib/craft-shared/agent/mode-types'
import type { ThinkingLevel } from "@/lib/craft-shared/agent/thinking-levels"
import {
  TurnCard,
  UserMessageBubble,
  groupMessagesByTurn,
...
import { ChatInputZone, type StructuredInputState, type StructuredResponse, type PermissionResponse, type AdminApprovalResponse } from '@/components/input'
...
import { resolveBranchNewPanelOption } from '@/components/shell/branching'
...
interface ChatDisplayProps {
  ...
  onRespondToPermission?: (
    ...
    allowed: boolean,
    alwaysAllow: boolean,
    options?: import('@/lib/craft-shared/types').PermissionResponseOptions
  ) => void
  ...
  labels?: import('@/lib/craft-shared/labels').LabelConfig[]
...
export const ChatDisplay = React.forwardRef<ChatDisplayHandle, ChatDisplayProps>(function ChatDisplay({
  ...
  const scrollViewportRef = React.useRef<HTMLDivElement>(null) as React.RefObject<HTMLDivElement>
          ...
          <div className="mt-2">
            ...
            <AnimatedCollapsibleContent isOpen={detailsOpen} className="overflow-hidden">
              <div className="mt-2 pt-2 border-t border-destructive/20 text-xs text-destructive/60 font-mono space-y-0.5">
                {(message.errorDetails as string[] | undefined)?.map((detail: string, i: number) => (
                  <div key={i}>{detail}</div>
                ))}
                {message.errorOriginal && !(message.errorDetails as string[] | undefined)?.some((d: string) => d.includes('Raw error:')) && (
...
function MessageBubble({
  ...
  if (message.role === 'info') {
    ...
    const level = (message.infoLevel || 'info') as 'info' | 'warning' | 'error' | 'success'
    const configMap = {
