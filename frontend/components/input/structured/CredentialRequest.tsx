...
import type { CredentialRequest as CredentialRequestType, CredentialResponse } from '@/lib/craft-shared/types'
...
export function CredentialRequest({ request, onResponse, unstyled = false }: CredentialRequestProps) {
  ...
  const isValid = isBasicAuth
    ...
    : isMultiHeader
    ? request.headerNames?.every((name: string) => headerValues[name]?.trim().length > 0) ?? false
    : value.trim().length > 0
        ...
        <div className="p-4 space-y-4 flex-1 min-h-0 flex flex-col">
          ...
          <div className="space-y-3">
            {isBasicAuth ? (
              ...
              <>
                {request.headerNames.map((headerName: string, index: number) => (
