...
import { INLINE_MATH_EDIT_EVENT, TiptapBubbleMenus } from "./TiptapBubbleMenus";
...
const CURRENCY_RANGE_REGEX =
	/\$(\d[\dA-Za-z.,]*\s*[–-]\s*)\$(\d[\dA-Za-z.,]*)/g;
...
export function preprocessMarkdownForOfficial(markdown: string): string {
	...
	const rangeProtected = withPlaceholders.replace(
		...
		(_match, left: string, right: string) =>
			`${CURRENCY_MARKER}${left}${CURRENCY_MARKER}${right}`,
	);
...
export function extractMermaidSource(text: string): string | null {
	...
	const looksLikeMermaid = MERMAID_DIAGRAM_PREFIXES.some((prefix) =>
		firstMeaningful.startsWith(prefix),
	);
		...
		const base = [
			...
			FileHandler.configure({
				onPaste: async (editor, files) => {
					...
					await handleDroppedOrPastedFiles(
						editor as NonNullable<ReturnType<typeof useEditor>>,
						files,
					);
				},
			...
			onUpdate: ({ editor }) => {
				const md = useOfficialMarkdown
					? postprocessMarkdownFromOfficial(
							getOfficialMarkdown(editor as { getMarkdown?: () => string }),
						)
					: getLegacyMarkdown(
							editor as {
								storage: { markdown?: { getMarkdown?: () => string } };
