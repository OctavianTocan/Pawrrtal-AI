...
function createComponents(
	...
	if (mode === "minimal") {
		return {
				...
				if (match || isBlock) {
					...
					if (match?.[1] === "diff") {
						return wrapBlock(
							"code",
							code,
							<MarkdownDiffBlock code={code} className="my-2" />,
							props.node?.position,
						);
					}
					...
					if (match?.[1] === "json") {
						return wrapBlock(
							"code",
							code,
							<MarkdownJsonBlock code={code} className="my-2" />,
							props.node?.position,
						);
					}
					...
					if (match?.[1] === "datatable") {
						return wrapBlock(
							"datatable",
							code,
							<MarkdownDatatableBlock code={code} className="my-2" />,
							props.node?.position,
						);
					}
					...
					if (match?.[1] === "spreadsheet") {
						return wrapBlock(
							"spreadsheet",
							code,
							<MarkdownSpreadsheetBlock code={code} className="my-2" />,
							props.node?.position,
						);
					}
					...
					if (match?.[1] === "html-preview") {
						return wrapBlock(
							"html-preview",
							code,
							<MarkdownHtmlBlock code={code} className="my-2" />,
							props.node?.position,
						);
					}
					...
					if (match?.[1] === "pdf-preview") {
						return wrapBlock(
							"pdf-preview",
							code,
							<MarkdownPdfBlock code={code} className="my-2" />,
							props.node?.position,
						);
					}
					...
					if (match?.[1] === "image-preview") {
						return wrapBlock(
							"image-preview",
							code,
							<MarkdownImageBlock code={code} className="my-2" />,
							props.node?.position,
						);
					}
				...
				<ul
					className={cn(
						...
						className?.includes("contains-task-list") &&
							"list-none ps-0 marker:content-none",
					)}
				>
					{children}
				</ul>
			...
			ol: ({ children, className }) => (
				<ol className={cn("my-2 space-y-1 pl-6 list-decimal", className)}>
					{children}
				</ol>
			),
			li: ({ children, className }) => (
				<li
					className={cn(className?.includes("task-list-item") && "list-none")}
				>
					{children}
				</li>
			),
			...
			h1: ({ children }) => (
				<h1 className="font-sans text-[16px] font-bold mt-5 mb-3">
					{children}
				</h1>
			),
			h2: ({ children }) => (
				<h2 className="font-sans text-[16px] font-semibold mt-4 mb-3">
					{children}
				</h2>
			),
			h3: ({ children }) => (
				<h3 className="font-sans text-[15px] font-semibold mt-4 mb-2">
					{children}
				</h3>
			),
			// Blockquotes
	...
	return {
		...
		ol: ({ children, className }) => (
			<ol className={cn("my-3 space-y-1.5 pl-6 list-decimal", className)}>
				{children}
			</ol>
		),
		li: ({ children, className }) => (
		...
		h2: ({ children }) => (
			<h2 className="font-sans text-[16px] font-semibold mt-6 mb-3">
				{children}
			</h2>
		),
		h3: ({ children }) => (
			<h3 className="font-sans text-[15px] font-semibold mt-5 mb-3">
				{children}
			</h3>
		),
		h4: ({ children }) => (
