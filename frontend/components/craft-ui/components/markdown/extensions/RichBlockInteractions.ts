...
const RICH_BLOCK_SELECTION_HIGHLIGHT_KEY = new PluginKey<{
	suspendDuringPointerDrag: boolean;
}>("richBlockSelectionHighlight");
...
function isEditableRichBlockNode(
	node: { type?: { name?: string } } | null | undefined,
): boolean {
	return (
		node?.type?.name != null && EDITABLE_RICH_BLOCK_NODES.has(node.type.name)
	);
}
		...
		return [
			new Plugin({
				...
				state: {
					...
					apply(tr, prev) {
						const meta = tr.getMeta(RICH_BLOCK_SELECTION_HIGHLIGHT_KEY) as
							| { suspendDuringPointerDrag?: boolean }
							| undefined;
						if (
							typeof meta?.suspendDuringPointerDrag === "boolean" &&
							meta.suspendDuringPointerDrag !== prev.suspendDuringPointerDrag
						) {
							return {
								suspendDuringPointerDrag: meta.suspendDuringPointerDrag,
							};
						}
						return prev;
					},
				...
				view: (view) => {
					...
					const setSuspendDuringPointerDrag = (
						suspendDuringPointerDrag: boolean,
					) => {
						const pluginState = RICH_BLOCK_SELECTION_HIGHLIGHT_KEY.getState(
							view.state,
						) as { suspendDuringPointerDrag: boolean } | undefined;
						if (
							pluginState?.suspendDuringPointerDrag === suspendDuringPointerDrag
						)
							return;
						view.dispatch(
							view.state.tr.setMeta(RICH_BLOCK_SELECTION_HIGHLIGHT_KEY, {
								suspendDuringPointerDrag,
							}),
						);
					};
				...
				props: {
					...
					handleKeyDown: (view, event) => {
						if (event.key === "Enter") {
							...
							if (
								selection instanceof NodeSelection &&
								isEditableRichBlockNode(selection.node)
							) {
								event.preventDefault();
								(this.editor as any).emit(RICH_BLOCK_EDIT_EVENT);
								return true;
						...
						if (event.key !== "ArrowDown" && event.key !== "ArrowUp")
							return false;
						if (isSlashSuggestionActive(this.editor as CoreEditor))
							return false;
						...
						if (
							selection instanceof NodeSelection &&
							isRichBlockNode(selection.node)
						) {
							event.preventDefault();
