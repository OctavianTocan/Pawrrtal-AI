...
function findQuoteRange(
	...
	while (searchIndex !== -1) {
		...
		const prefixOk =
			!quote.prefix ||
			fullText.slice(
				Math.max(0, candidateStart - quote.prefix.length),
				candidateStart,
			) === quote.prefix;
		const suffixOk =
			!quote.suffix ||
			fullText.slice(candidateEnd, candidateEnd + quote.suffix.length) ===
				quote.suffix;
	...
	const normalizedPrefix = quote.prefix
		? normalizeWhitespaceWithMap(quote.prefix).text
		: undefined;
	const normalizedSuffix = quote.suffix
		? normalizeWhitespaceWithMap(quote.suffix).text
		: undefined;
	...
	while (normalizedIndex !== -1) {
		...
		const prefixOk =
			!normalizedPrefix ||
			normalizedFull.text.slice(
				Math.max(0, normalizedIndex - normalizedPrefix.length),
				normalizedIndex,
			) === normalizedPrefix;
		const suffixOk =
			!normalizedSuffix ||
		...
		normalizedIndex = normalizedFull.text.indexOf(
			normalizedExact,
			normalizedIndex + 1,
		);
	}
...
export function resolveTextAnnotations(
	...
	for (const annotation of annotations) {
		...
		const position = selectors.find((s) => s.type === "text-position") as
			| Extract<
					AnnotationV1["target"]["selectors"][number],
					{ type: "text-position" }
			  >
		...
		const quote = selectors.find((s) => s.type === "text-quote") as
			| Extract<
					AnnotationV1["target"]["selectors"][number],
					{ type: "text-quote" }
			  >
