...
function formatCell(value: unknown, type?: ColumnDef["type"]): React.ReactNode {
	...
	switch (type) {
		...
		case "badge": {
			...
			const color =
				s === "active" || s === "passing" || s === "success" || s === "done"
...
function SortIcon({ dir }: { dir: SortDir }) {
	return (
		<svg
			className={cn("w-3 h-3 shrink-0", dir ? "opacity-60" : "opacity-20")}
			viewBox="0 0 16 16"
			fill="currentColor"
		>
			{dir === "asc" ? (
...
function computeNumericGranularities(
	values: number[],
...
function computeGranularityOptions(
	data: DatatableData,
): Map<string, GranularityOption[]> {
...
> {
	...
	componentDidCatch(error: Error) {
		console.warn(
			"[MarkdownDatatableBlock] Render failed, falling back to CodeBlock:",
			error,
		);
	}
	render() {
...
export function MarkdownDatatableBlock({
	code,
	className,
}: MarkdownDatatableBlockProps) {
	...
	React.useEffect(() => {
		...
		onReadFile(spec.src)
			...
			.catch((err) => {
				setFileError(
					err instanceof Error ? err.message : "Failed to read data file",
				);
			})
	...
	const [groupGranularity, setGroupGranularity] =
		React.useState<string>("exact");
	...
	const hasActiveControls =
		(sortKey !== null && sortDir !== null) || groupKey !== null;
	...
	if (spec?.src && fileLoading) {
		return (
			<div
				className={cn(
					"rounded-[8px] overflow-hidden border bg-muted/10",
					className,
				)}
			>
				<div className="px-3 py-2 bg-muted/50 border-b">
					<span className="text-[12px] text-muted-foreground font-medium">
						{spec.title || "Data Table"}
					</span>
				</div>
	...
	if (spec?.src && fileError) {
		return (
			...
			>
				...
				<div className="py-6 text-center text-destructive/70 text-[13px]">
					{fileError}
				</div>
			</div>
		);
	...
	const fallback = (
		<CodeBlock code={code} language="json" mode="full" className={className} />
	);

	const groupColumnLabel = groupKey
		? (parsed.columns.find((c) => c.key === groupKey)?.label ?? groupKey)
		: "";
	...
	const renderRows = (rows: Record<string, unknown>[]) =>
		rows.map((row, i) => (
			<tr
				key={i}
				className="border-b border-foreground/[0.03] last:border-0 hover:bg-foreground/[0.015] transition-colors"
			>
					...
					<tr className="border-b border-foreground/[0.06] bg-foreground/[0.02]">
						{parsed.columns.map((col) => (
							<th
								key={col.key}
								className={cn(
									"py-2 px-3 text-[12px] cursor-pointer select-none whitespace-nowrap",
									colAlign(col.type, col.align),
								)}
				...
				<tbody>
					{groupedData ? (
							...
							<React.Fragment key={group.value}>
								...
								>
									<td
										...
										className="py-2 px-3 bg-foreground/[0.03] border-b border-foreground/[0.06]"
									>
										<span className="inline-flex items-center gap-2 text-[12px] font-medium text-muted-foreground">
											<ChevronRight
												className={cn(
													"w-3 h-3 transition-transform",
													!collapsedGroups.has(group.value) && "rotate-90",
												)}
											/>
						...
						<tr>
							<td
								colSpan={parsed.columns.length}
								className="py-6 text-center text-muted-foreground text-[13px]"
							>
								No rows
							</td>
						</tr>
							...
							if (opts && opts.length > 0) {
								return (
									<DropdownMenuSub key={`group-${col.key}`}>
										...
										<StyledDropdownMenuSubContent
											style={{
												zIndex: "calc(var(--z-floating-menu, 400) + 2)",
											}}
										>
											{opts.map((opt) => {
												const optActive =
													isActive && groupGranularity === opt.value;
												return (
													<StyledDropdownMenuItem
														key={opt.value}
														onSelect={(e) => {
													...
													>
														<span
															className={cn(
																"flex-1",
																optActive && "text-accent font-medium",
															)}
														>
															{opt.label}
														</span>
	...
	return (
		<DatatableErrorBoundary fallback={fallback}>
			<div
				className={cn(
					"relative group rounded-[8px] overflow-hidden border bg-muted/10",
					className,
				)}
			>
				{/* Control button */}
				...
				headerActions={
					<div className="flex items-center gap-1.5">
						...
						<TableExportDropdown
							...
							rows={parsed.rows}
							filename={parsed.title || "Data Table"}
						/>
					</div>
				}
			>
