...
const LUCIDE_ICON_NODES: Record<
	SlashIconName,
...
function resolvePreferredCodeLanguage(editor: Editor): string {
	...
	if (editor.isActive("codeBlock")) {
		const current = String(editor.getAttributes("codeBlock").language ?? "")
			.trim()
			.toLowerCase();
...
function insertCodeBlockWithPlaceholder(editor: Editor, insertPos?: number) {
	...
	editor
		...
		.setTextSelection({ from: targetPos + 1, to: targetPos + 1 })
		.run();
...
function getRectFromClientRect(
