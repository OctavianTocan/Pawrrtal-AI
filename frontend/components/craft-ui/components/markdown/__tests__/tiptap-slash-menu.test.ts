...
describe("tiptap slash menu", () => {
	it("filters by title, description, and aliases", () => {
		...
		expect(
			filterSlashCommandItems(items, "heading").some(
				(item) => item.id === "heading-1",
			),
		).toBe(true);
		expect(
			filterSlashCommandItems(items, "divider").some(
				(item) => item.id === "horizontal-rule",
			),
		).toBe(true);
		expect(
			filterSlashCommandItems(items, "flow").some(
				(item) => item.id === "mermaid-code-block",
			),
		).toBe(true);
		expect(
			filterSlashCommandItems(items, "checklist").some(
				(item) => item.id === "task-list",
			),
		).toBe(true);
	});
	...
	it("returns all items for empty query and includes icons", () => {
		...
		expect(
			filtered.some((item) => (item as { group: string }).group === "Visual"),
		).toBe(false);
	});
	...
	it("maps commands to expected chain calls", async () => {
		...
		expect(calls).toContain(
			'insertContentAt:5:{"type":"codeBlock","attrs":{"language":"plaintext"},"content":[{"type":"text","text":" "}]}',
		);
		...
		expect(calls).toContain(
			'insertContentAt:5:{"type":"mermaidBlock","attrs":{"code":"graph TD\\n  A[Start] --> B[End]"}}',
		);
		...
		expect(calls).toContain(
			'insertContentAt:5:{"type":"latexBlock","attrs":{"code":"E = mc^2"}}',
		);
	});
});
