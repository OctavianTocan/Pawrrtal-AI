...
describe("wrapWithSafeProxy", () => {
	...
	describe("returns fallback for invalid tag names", () => {
		...
		it("returns fallback function for tag with hyphen", () => {
			expect(typeof getComponent(safeComponents, "my-element")).toBe(
				"function",
			);
		});
	...
	describe("fallback component behavior", () => {
		it("fallback is callable and returns a React element", () => {
			const Fallback = getComponent(safeComponents, "sq+qr") as (props: {
				children: string;
			}) => unknown;
...
describe("wrapWithSafeProxy getOwnPropertyDescriptor trap", () => {
	...
	describe("Object.hasOwnProperty behavior", () => {
		it("returns true for defined components", () => {
			expect(Object.hasOwn(safeComponents, "code")).toBe(true);
		});

		it("returns false for valid but undefined tags", () => {
			expect(Object.hasOwn(safeComponents, "div")).toBe(false);
			expect(Object.hasOwn(safeComponents, "span")).toBe(false);
			expect(Object.hasOwn(safeComponents, "MyComponent")).toBe(false);
		});

		it("returns true for invalid tag names (so fallback is used)", () => {
			expect(Object.hasOwn(safeComponents, "sq+qr")).toBe(true);
			expect(Object.hasOwn(safeComponents, "SQ+QR")).toBe(true);
			expect(Object.hasOwn(safeComponents, "P 150")).toBe(true);
			expect(Object.hasOwn(safeComponents, "@mention")).toBe(true);
			expect(Object.hasOwn(safeComponents, "my-element")).toBe(true);
		});
	});
...
describe("actual crash case: QR code label text", () => {
	...
	it("complete flow: invalid tag in components check then get", () => {
		...
		const Component = getComponent(safeComponents, tagName) as (props: {
			children: string;
		}) => unknown;
