...
describe("preprocessLinks", () => {
	describe("preserves existing markdown links", () => {
		it("does not wrap a domain inside markdown link text", () => {
			const input =
				"- [help.figma.com - Pan and zoom in FigJam](https://help.figma.com/hc/en-us/articles/123)";
		...
		it("preserves multiple markdown links in the same text", () => {
			const input =
				"See [docs.github.com - Actions](https://docs.github.com/actions) and [api.stripe.com - Charges](https://api.stripe.com/charges)";
			expect(preprocessLinks(input)).toBe(input);
		});
		...
		it("preserves link with domain and extra description in text", () => {
			const input =
				"- [stackoverflow.com - How to fix React hydration errors](https://stackoverflow.com/questions/123)";
			expect(preprocessLinks(input)).toBe(input);
		});
	});
	...
	describe("still wraps bare URLs that are not already linked", () => {
		...
		it("wraps a bare repo-relative file path", () => {
			const input =
				"See apps/electron/resources/docs/browser-tools.md for details";
			expect(preprocessLinks(input)).toBe(
				"See [apps/electron/resources/docs/browser-tools.md](apps/electron/resources/docs/browser-tools.md) for details",
			);
		});
		...
		it("wraps a bare domain", () => {
			...
			expect(preprocessLinks(input)).toBe(
				"Check out [example.com](http://example.com) for details",
			);
		});

		it("wraps bare URL but preserves adjacent markdown link", () => {
			const input =
				"See https://bare.example.com and [linked.example.com - Title](https://linked.example.com/page)";
			...
			expect(result).toContain(
				"[linked.example.com - Title](https://linked.example.com/page)",
			);
		});
	});
	...
	describe("strips trailing markdown formatting from URLs", () => {
		it("does not include trailing ** from bold-wrapped URL", () => {
			const input =
				"PR created: **https://github.com/lukilabs/craft-growth/pull/1363**";
			const result = preprocessLinks(input);
			expect(result).toBe(
				"PR created: **[https://github.com/lukilabs/craft-growth/pull/1363](https://github.com/lukilabs/craft-growth/pull/1363)**",
			);
		});
		...
		it("handles bold-wrapped URL with path and trailing text", () => {
			...
			const result = preprocessLinks(input);
			expect(result).toBe(
				"See **[https://github.com/org/repo/pull/42](https://github.com/org/repo/pull/42)** for details",
			);
		});
	});
...
describe("preprocessLinks — placeholder URL stripping", () => {
	it("strips GitHub link with /... placeholder to plain text", () => {
		const input =
			"**[6610172ec](https://github.com/...) - feat: Browse cache (#6644)**";
		const result = preprocessLinks(input);
	...
	it("strips any link with /... in the URL path", () => {
		const input =
			"See [commit abc123](https://github.com/.../commit/abc123) for details";
		const result = preprocessLinks(input);
	...
	it("handles multiple links where some are placeholders", () => {
		const input =
			"See [real link](https://github.com/org/repo/issues/1) and [fake link](https://github.com/...)";
		const result = preprocessLinks(input);
		expect(result).toContain(
			"[real link](https://github.com/org/repo/issues/1)",
		);
...
describe("isPlaceholderUrl", () => {
	...
	it("does not flag valid GitHub URLs", () => {
		...
		expect(isPlaceholderUrl("https://github.com/org/repo/commit/abc123")).toBe(
			false,
		);
	});

	it("does not flag URLs with triple dots in query params", () => {
		expect(isPlaceholderUrl("https://example.com/search?q=test...more")).toBe(
			false,
		);
	});
	...
	it("does not flag compare URLs with two dots", () => {
		expect(
			isPlaceholderUrl("https://github.com/org/repo/compare/main..feature"),
		).toBe(false);
	});
	...
	it("does not flag three-dot GitHub compare URLs", () => {
		...
		expect(
			isPlaceholderUrl("https://github.com/org/repo/compare/v1.0.0...v2.0.0"),
		).toBe(false);
	});
});
...
describe("detectLinks", () => {
	...
	it("detects bare repo-relative file paths", () => {
		const links = detectLinks(
			"Open apps/electron/resources/docs/browser-tools.md",
		);
		expect(links).toHaveLength(1);
		expect(links[0]).toBeDefined();
		expect(links[0]!.type).toBe("file");
...
describe("isFilePathTarget", () => {
	it("accepts absolute unix image paths", () => {
		expect(
			isFilePathTarget(
				"/Users/balintorosz/.craft-agent/sessions/abc/image.jpg",
			),
		).toBe(true);
	});
	...
	it("accepts repo-relative markdown paths", () => {
		expect(
			isFilePathTarget("apps/electron/resources/docs/browser-tools.md"),
		).toBe(true);
	});
