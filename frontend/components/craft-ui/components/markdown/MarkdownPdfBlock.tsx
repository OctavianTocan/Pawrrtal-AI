...
> {
	...
	componentDidCatch(error: Error) {
		console.warn(
			"[MarkdownPdfBlock] Render failed, falling back to CodeBlock:",
			error,
		);
	}
	render() {
		...
		onReadFileBinary(activeItem.src)
			.then((data) => {
				...
				setContentCache((prev) => ({
					...prev,
					[activeItem.src]: new Uint8Array(data),
				}));
			})
			.catch((err) => {
				setError(
				...
				<div className="relative h-[400px] overflow-hidden">
					...
					{activePdfData && (
						<div
							...
							style={{
								background:
									"linear-gradient(to bottom, transparent, var(--muted))",
