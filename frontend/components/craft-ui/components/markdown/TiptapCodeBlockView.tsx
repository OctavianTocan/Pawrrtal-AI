...
function buildLanguageOptions(): CodeLanguageOption[] {
	const allSupported = Object.keys(bundledLanguages).filter(
		(lang) => !EXCLUDED_LANGUAGES.has(lang),
	);

	const prioritized = PRIORITY_LANGUAGES.filter((lang) =>
		allSupported.includes(lang),
	);
...
function getLanguageLabel(language: string | null | undefined): string {
	...
	const found = CODE_LANGUAGE_OPTIONS.find(
 ...
 * - latexBlock
 */
function TiptapCodeBlockView({
	node,
	updateAttributes,
}: TiptapCodeBlockViewProps) {
	...
	const [highlightedLanguageIndex, setHighlightedLanguageIndex] =
		React.useState(0);
	...
	const currentLanguage = normalizeLanguage(
		(node.attrs.language as string | undefined) ?? null,
	);
	...
	React.useEffect(() => {
		...
		const selectedIndex = filteredLanguageOptions.findIndex(
	...
	React.useEffect(() => {
		...
		setHighlightedLanguageIndex(
			Math.max(0, filteredLanguageOptions.length - 1),
		);
	}, [filteredLanguageOptions.length, highlightedLanguageIndex]);

	React.useEffect(() => {
		if (!languageDropdownOpen) return;
		languageOptionRefs.current[highlightedLanguageIndex]?.scrollIntoView({
			block: "nearest",
		});
	}, [languageDropdownOpen, highlightedLanguageIndex, filteredLanguageOptions]);

	const handleLanguageFilterKeyDown = React.useCallback(
		(event: React.KeyboardEvent<HTMLInputElement>) => {
			...
			if (event.key === "ArrowDown") {
				...
				setHighlightedLanguageIndex(
					(prev) => (prev + 1) % filteredLanguageOptions.length,
				);
				return;
			}
			...
			if (event.key === "ArrowUp") {
				...
				setHighlightedLanguageIndex(
					(prev) =>
						(prev - 1 + filteredLanguageOptions.length) %
	...
	return (
		<NodeViewWrapper
			...
			data-actions-open={languageDropdownOpen ? "true" : "false"}
		>
			<TiptapHoverActions>
				<SimpleDropdown
					align="end"
				...
				>
					...
					<div className="max-h-[240px] overflow-y-auto p-1">
						{filteredLanguageOptions.length === 0 ? (
							<div className="px-2.5 py-2 text-xs text-foreground/50 select-none">
								No languages found
							</div>
						) : (
