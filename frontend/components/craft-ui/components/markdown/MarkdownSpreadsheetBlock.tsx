...
> {
	...
	componentDidCatch(error: Error) {
		console.warn(
			"[MarkdownSpreadsheetBlock] Render failed, falling back to CodeBlock:",
			error,
		);
	}
	render() {
...
export function MarkdownSpreadsheetBlock({
	code,
	className,
}: MarkdownSpreadsheetBlockProps) {
	...
	if (spec?.src && fileLoading) {
		const loadingLabel =
			[spec.filename, spec.sheetName].filter(Boolean).join(" — ") ||
			"Spreadsheet";
		return (
			<div
				className={cn(
	...
	const label =
		[parsed.filename, parsed.sheetName].filter(Boolean).join(" — ") ||
		"Spreadsheet";
	const fallback = (
				...
				<thead>
					<tr className="border-b border-foreground/[0.08] bg-foreground/[0.03]">
						...
						{colLetters.map((letter) => (
							<th
								key={letter}
								className="text-center py-1 px-3 font-normal text-muted-foreground/40 border-r border-foreground/[0.06] last:border-0 text-[11px]"
							>
								{letter}
							</th>
						))}
					...
					<tr className="border-b border-foreground/[0.06] bg-foreground/[0.02]">
						<td className="text-center py-1.5 px-2 text-muted-foreground/40 border-r border-foreground/[0.06] text-[11px] font-mono">
							1
						</td>
						{parsed.columns.map((col) => (
							<td
								key={col.key}
								className="py-1.5 px-3 font-semibold text-foreground border-r border-foreground/[0.06] last:border-0"
							>
								{col.label}
							</td>
						))}
			...
			<DataTableOverlay
				...
				headerActions={
					<TableExportDropdown
						...
						rows={parsed.rows}
						filename={parsed.filename || parsed.sheetName || "Spreadsheet"}
					/>
				}
			>
				<div className="px-6">
