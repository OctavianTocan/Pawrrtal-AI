...
> {
	...
	componentDidCatch(error: Error) {
		console.warn(
			"[MarkdownImageBlock] Render failed, falling back to CodeBlock:",
			error,
		);
	}
	render() {
...
export function MarkdownImageBlock({
	code,
	className,
	onCreateRegionAnnotation: _onCreateRegionAnnotation,
}: MarkdownImageBlockProps) {
	...
	const activeDataUrl = safeActiveItem
		? contentCache[safeActiveItem.src]
		: undefined;
	...
	const handleLoadDataUrl = React.useCallback(
		async (path: string) => {
	...
	const stackItems = items.reduce<
		Array<{ src: string; label?: string; ratio?: number; alt: string }>
	>((acc, item, index) => {
	...
	return (
		<ImageBlockErrorBoundary fallback={fallback}>
			<div
				className={cn(
					"relative group rounded-[8px] overflow-visible",
					className,
				)}
			>
				<div className="relative h-[320px] overflow-visible flex items-center justify-center p-3">
					...
					{loading &&
						(!activeDataUrl || (hasMultiple && stackItems.length === 0)) && (
