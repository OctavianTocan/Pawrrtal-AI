 ...
 */
export function getDiffStats(fileDiff: FileDiffMetadata): {
	additions: number;
	deletions: number;
} {
	let additions = 0;
...
export function ShikiDiffViewer({
	...
	const resolvedThemeName =
		shikiTheme || (theme === "dark" ? "craft-dark" : "craft-light");
	...
	useEffect(() => {
		...
		const timer = setTimeout(() => {
			const diffsContainer =
				containerRef.current?.querySelector(DIFFS_TAG_NAME);
			const header = diffsContainer?.shadowRoot?.querySelector(
				"[data-diffs-header]",
			);
			if (!header) return;
