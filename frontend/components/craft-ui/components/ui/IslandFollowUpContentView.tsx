...
export function IslandFollowUpContentView({
	...
	const handleSubmitMenuInteractOutside = React.useCallback(
		(event: unknown) => {
	...
	React.useLayoutEffect(() => {
		...
		const nextHeight = Math.min(
			Math.max(measured, minInputHeight),
			maxInputHeight,
		);
	...
	return (
		<IslandContentView
			...
			blockOutsideInteraction={blockOutsideInteraction}
		>
			<div className="w-[330px] px-3 pb-3 pt-3 space-y-2.5 select-none">
				...
				<div className="relative rounded-[8px] px-0 py-1">
					...
					<textarea
						...
						style={{
							height: inputHeight,
							overflowY: inputOverflow ? "auto" : "hidden",
						}}
						...
						{canSubmitAndSend ? (
							<div className="inline-flex rounded-[8px] bg-background shadow-minimal overflow-hidden">
								...
								<DropdownMenu
									open={submitMenuOpen}
									onOpenChange={(open) => {
										if (!isEmpty) setSubmitMenuOpen(open);
									}}
								>
