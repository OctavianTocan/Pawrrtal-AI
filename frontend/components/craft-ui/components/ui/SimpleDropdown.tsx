...
const SimpleDropdownContext =
	React.createContext<SimpleDropdownContextValue | null>(null);
...
export function SimpleDropdown({
	...
	const setIsOpenWithCallback = useCallback(
	...
	const setItemRef = useCallback(
		(id: string, el: HTMLButtonElement | null) => {
			...
			itemOrder.current = itemOrder.current.filter(
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (e.key === "ArrowDown" || e.key === "ArrowUp") {
				...
				const nextIndex =
					currentIndex < 0
						? 0
						: (currentIndex + delta + navigableIds.length) %
							navigableIds.length;
