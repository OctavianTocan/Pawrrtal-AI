...
const SUPPORTED_HOVER_PREFIXES = [
	"bg-",
	"text-",
	"border-",
	"ring-",
 ...
 */
export function mirrorHoverToOpenStateClasses(
	className?: string,
): string | undefined {
	...
	for (const token of tokens) {
		...
		const utility = afterHover.includes(":")
			? afterHover.slice(afterHover.lastIndexOf(":") + 1)
			: afterHover;
...
>(
	(
		{ className, autoMirrorHoverToOpen = true, asChild, children, ...props },
		ref,
	) => {
		const triggerClassName = autoMirrorHoverToOpen
			? mirrorHoverToOpenStateClasses(className)
			: className;

		if (asChild && autoMirrorHoverToOpen && React.isValidElement(children)) {
			const childClassName = (children.props as { className?: string })
				.className;
			const mergedChildClassName = mirrorHoverToOpenStateClasses(
				cn(childClassName, className),
			);
