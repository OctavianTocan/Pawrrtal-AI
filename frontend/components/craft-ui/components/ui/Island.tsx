...
const IslandAnimationContext =
	React.createContext<Required<IslandTransitionConfig>>(DEFAULT_TRANSITION);
...
function computeDirectionalOffset(
	angleDeg: number,
	distancePx: number,
): { x: number; y: number } {
...
function computeMorphDelta(
	...
	const baseWidth =
		elementLayoutWidth > 0 ? elementLayoutWidth : elementRect.width;
	const baseHeight =
		elementLayoutHeight > 0 ? elementLayoutHeight : elementRect.height;

	return {
		x: targetCenterX - elementCenterX,
		y: targetCenterY - elementCenterY,
		scaleX: clampScale(
			target.width != null && baseWidth > 0 ? target.width / baseWidth : 0.16,
		),
		scaleY: clampScale(
			target.height != null && baseHeight > 0
...
export function Island({
	...
	const layerIdRef = React.useRef(
		`island-${Math.random().toString(36).slice(2)}`,
	);
	...
	const [morphDelta, setMorphDelta] = React.useState<{
		x: number;
		y: number;
		scaleX: number;
		scaleY: number;
	...
	const shouldPrimeInitialVisibleReplay =
		replayOnVisible === "always" && isVisible;
	const [isVisibilityPrimed, setIsVisibilityPrimed] = React.useState(
		() => !shouldPrimeInitialVisibleReplay,
	);
	const prevVisibilityForReplayRef = React.useRef<boolean>(
		shouldPrimeInitialVisibleReplay ? false : isVisible,
	);
	const prevReplayEntryKeyRef = React.useRef<string | number | undefined>(
		replayEntryKey,
	);
	const hasRenderedVisibleRef = React.useRef<boolean>(false);
	const spawnHiddenPoseRef = React.useRef<{
		opacity: number;
		x: number;
		y: number;
		scaleX: number;
	...
	React.useEffect(() => {
		...
		const replayKeyChangedWhileVisible =
			isVisible && prevReplayEntryKeyRef.current !== replayEntryKey;
		...
		const needsPriming =
			becameVisible || replayKeyChangedWhileVisible || !isVisibilityPrimed;
		if (!needsPriming) {
			return;
		}
	...
	const shouldLockScroll =
		(activeView?.lockScroll ?? false) ||
		(activeView?.blockOutsideInteraction ?? false) ||
		lockScrollWhileVisible;
	...
	const hasEntryScale =
		transitionConfig?.entryStartScale != null &&
		Math.abs(cfg.entryStartScale - 1) > 0.0001;
	const hasReplayEntryRequest = replayEntryKey != null;
	const shouldAnimateFromHiddenOnMount =
		shouldMorph ||
		hasEntryTranslation ||
		hasEntryScale ||
		hasReplayEntryRequest;
	const shouldUseConfiguredStartScale =
		transitionConfig?.entryStartScale != null;
	...
	const hiddenPose = {
		...
		scaleX:
			hasUsableMorphDelta && !shouldUseConfiguredStartScale
	...
	const shouldHideForWarmup =
		shouldMorph &&
		isVisible &&
		!isMorphWarmReady &&
		!hasRenderedVisibleRef.current;
	const shouldHideForReplayPriming =
		replayOnVisible === "always" && isVisible && !isVisibilityPrimed;
	const effectiveVisible =
		isVisible && !shouldHideForWarmup && !shouldHideForReplayPriming;
	const shouldBlockOutside =
		(activeView?.blockOutsideInteraction ?? false) && effectiveVisible;
	...
	return (
		<IslandAnimationContext.Provider value={cfg}>
			...
			<motion.div
				...
				className={cn(
					"mx-auto w-fit overflow-hidden border border-border/50 bg-background shadow-strong",
					className,
				)}
			>
					...
					<AnimatePresence initial={false} mode="popLayout">
						...
						>
							<div
								...
								className={cn(
									"flex",
									resolveAlignClass(activeView.anchorX, activeView.anchorY),
