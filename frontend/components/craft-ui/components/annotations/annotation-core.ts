...
export function hasExistingTextRangeAnnotation(
	...
	return (annotations ?? []).some((annotation) => {
		const pos = annotation.target.selectors.find(
			(s) => s.type === "text-position",
		) as
			| Extract<
...
export function collectTextSegments(
	...
	while (current) {
		...
		if (
			parent?.closest("[data-ca-annotation-overlay]") ||
			parent?.closest("[data-ca-annotation-index]")
		) {
...
export function resolveNodeOffset(
	...
	try {
		...
		fragment
			.querySelectorAll("[data-ca-annotation-index]")
...
export function resolveRangeFromOffsets(
	root: HTMLElement,
	start: number,
	end: number,
): Range | null {
...
export function getClientRectsForOffsets(
	root: HTMLElement,
	start: number,
	end: number,
): DOMRect[] {
...
export function consolidateRectsByLine(
