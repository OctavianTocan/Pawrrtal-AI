...
import {
	getAnnotationChipVisual,
	getAnnotationRectVisual,
	...
	return (
		...
		>
			...
			{chips.map((chip) => {
				...
				const tooltipText =
					chipAnnotation && getTooltipText
						? getTooltipText(chipAnnotation, chip.index)
						: "";
				...
				if (tooltipText) {
					return (
						<Tooltip key={`chip-${chip.id}`}>
							...
							<TooltipContent
								side="top"
								className="max-w-[280px] whitespace-pre-wrap text-xs"
							>
								{tooltipText}
							</TooltipContent>
						</Tooltip>
				...
				return (
					<React.Fragment key={`chip-${chip.id}`}>{chipButton}</React.Fragment>
				);
			})}
		</div>
	);
