...
export function useAnnotationInteractionController() {
	...
	const openFromSelection = React.useCallback(
	...
	const openFromAnnotation = React.useCallback(
		(
			...
			dispatch(
				annotationInteractionActions.openFromAnnotation(detail, noteText, mode),
			);
		},
		[],
	);
	...
	const consumeExternalOpenRequest = React.useCallback(
		(
			...
			const annotationIndex = params.annotations.findIndex(
