...
export function getAnnotationNoteText(annotation: AnnotationV1): string {
	const noteBody = annotation.body.find(
		(body): body is Extract<AnnotationV1["body"][number], { type: "note" }> =>
			body.type === "note",
	);
	...
	const metaText =
		typeof followUpMeta?.text === "string" ? followUpMeta.text.trim() : "";
	return metaText;
}

export function getAnnotationFollowUpState(
	annotation: AnnotationV1,
): AnnotationFollowUpState {
	...
	const sentAt =
		typeof followUpMeta.lastSentAt === "number"
			? followUpMeta.lastSentAt
			: typeof followUpMeta.sentAt === "number"
				? followUpMeta.sentAt
				: null;

	const sentTextRaw =
		typeof followUpMeta.lastSentText === "string"
			? followUpMeta.lastSentText
			: typeof followUpMeta.sentText === "string"
				? followUpMeta.sentText
				: "";
...
export function formatAnnotationFollowUpTooltipText(
	annotation: AnnotationV1,
	maxLength = 180,
): string {
