...
export function computeAnnotationOverlayGeometry({
	...
	const annotationIndexById = new Map(
		(persistedAnnotations ?? []).map((annotation, idx) => [
			annotation.id,
			idx + 1,
		]),
	);
	...
	for (const item of resolution.resolved) {
		...
		const rawRects = getClientRectsForOffsets(
			root,
			item.range.start,
			item.range.end,
		).map((rect) => ({
		...
		const annotationIndex =
			annotationIndexOverrides?.get(item.annotation.id) ??
			annotationIndexById.get(item.annotation.id);
		if (annotationIndex == null || lineRects.length === 0) {
			continue;
		}

		const minTop = Math.min(...lineRects.map((rect) => rect.top));
		const topRowRects = lineRects.filter(
			(rect) => Math.abs(rect.top - minTop) <= 2,
		);
