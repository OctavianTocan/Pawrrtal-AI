...
export function applyBlockAnnotationMarker(
	root: HTMLElement,
	annotation: AnnotationV1,
): void {
	const blockSelector = annotation.target.selectors.find(
		(selector) => selector.type === "block",
	) as
		| Extract<AnnotationV1["target"]["selectors"][number], { type: "block" }>
		| undefined;
	...
	target.style.boxShadow =
		"inset 0 0 0 1px color-mix(in srgb, var(--info) 22%, transparent)";
}
