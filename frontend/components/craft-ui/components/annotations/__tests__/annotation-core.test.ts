...
describe("annotation core helpers", () => {
	it("creates text selection annotation with follow-up metadata and session id", () => {
		...
		expect(noteBody && "text" in noteBody ? noteBody.text : "").toBe(
			"Need follow-up",
		);

		const followUp = (annotation.meta as Record<string, unknown> | undefined)
			?.followUp as Record<string, unknown> | undefined;
	...
	it("creates preview annotation marked ephemeral with explicit source", () => {
		...
		expect(preview.target.source).toEqual({
			sessionId: "session-preview",
			messageId: "msg-2",
		});
		expect((preview.meta as Record<string, unknown>)?.ephemeral).toBe(true);
		expect((preview.meta as Record<string, unknown>)?.source).toBe(
			"follow-up-selection-preview",
		);
	});

	it("keeps chip visuals consistent across pending and sent states", () => {
		const pending = getAnnotationChipVisual({
			pendingFollowUp: true,
			sentFollowUp: false,
		});
		const sent = getAnnotationChipVisual({
