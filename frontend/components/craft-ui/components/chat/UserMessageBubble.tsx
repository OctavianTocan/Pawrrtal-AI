...
import type { ContentBadge, StoredAttachment } from "@craft-agent/core";
...
function isEditRequestBadge(badge: ContentBadge): boolean {
	return (
		badge.type === "context" && !!badge.rawText?.includes("<edit_request>")
	);
}
...
function InlineFileBadge({
	...
	const tooltipPath = normalizePath(rawPath).replace(
		/^.*\.craft-agent\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/,
		"",
	);
...
export function UserMessageBubble({
	...
	if (hasEditRequestBadges) {
		...
		const sortedBadges = [...editRequestBadges].sort(
			(a, b) => b.start - a.start,
		);
		for (const badge of sortedBadges) {
			displayContent =
				displayContent.slice(0, badge.start) + displayContent.slice(badge.end);
		}
