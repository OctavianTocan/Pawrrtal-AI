...
import type { Message, MessageRole, StoredMessage } from "@craft-agent/core";
 ...
 */
export function getAssistantTurnUiKey(
	turn: AssistantTurn,
	index: number,
): string {
...
function messageToActivity(
	...
	const activity: ActivityItem = {
		...
		error: message.isError
			? stripErrorTags(message.toolResult || message.content)
			: undefined,
	...
	if (activity.parentId) {
		const parent = existingActivities.find(
			(a) => a.toolUseId === activity.parentId,
		);
 ...
 */
function extractTodosFromActivities(
	activities: ActivityItem[],
): TodoItem[] | undefined {
	...
	const todoWriteActivities = activities
		.filter(
			(a) =>
				a.toolName === "TodoWrite" && a.status === "completed" && a.content,
		)
...
export function groupMessagesByTurn(messages: Message[]): Turn[] {
	...
	for (const message of sortedMessages) {
		...
		if (
			message.role === "info" &&
			message.statusType === "compaction_complete"
		) {
			if (currentTurn) {
		...
		if (message.role === "assistant") {
			...
			if (message.isIntermediate || message.isPending) {
				...
				if (intermediateActivity.parentId) {
					const parent = currentTurn.activities.find(
						(a) => a.toolUseId === intermediateActivity.parentId,
					);
...
export function getActivitySummary(turn: AssistantTurn): string {
	const completed = turn.activities.filter(
...
export function formatTurnAsMarkdown(turn: AssistantTurn): string {
	...
	lines.push(
		`**Status:** ${turn.isComplete ? "Complete" : "In Progress"} · ${summary}`,
	);
	lines.push("");
...
export function isActivityGroup(
	item: ActivityItem | ActivityGroup,
 ...
 */
function extractTaskOutputData(
	activity: ActivityItem,
): TaskOutputData | undefined {
...
export function countTotalActivities(
	items: (ActivityItem | ActivityGroup)[],
): number {
	let count = 0;
