...
} from "../annotations/annotation-host-config";
import {
...
import {
	applyBlockAnnotationMarker,
	clearBlockAnnotationMarkers,
} from "../annotations/block-markers";
import {
...
function computeEditWriteDiffStats(
	...
	if (toolName === "Edit") {
		...
		if (toolInput.changes && Array.isArray(toolInput.changes)) {
			...
			for (const change of toolInput.changes as Array<{
				path?: string;
				diff?: string;
			}>) {
				if (change.diff) {
...
function formatToolInput(
	...
	if (isEditOrWrite && input.changes && Array.isArray(input.changes)) {
		...
		if (firstChange?.path) {
			const pathStr = stripSessionFolderPath(
				firstChange.path,
				sessionFolderPath,
			);
			parts.push(pathStr);
 ...
 */
function extractActionFromDisplayName(
	iconName: string,
	llmName: string,
): string {
			...
			>
				...
				{isMcpOrApiTool && !isBackgrounded && (
					<>
						...
						{activity.toolName === "mcp__session__call_llm" &&
						...
						{(intentOrDescription ||
							(displayMode === "detailed" && (toolSlug || inputSummary))) && (
							<span
								className={cn(
					...
					<span className="flex items-center gap-1.5 text-[10px] shrink-0">
						...
						{(() => {
							...
							if (typeof activity.toolInput?.file_path === "string") {
								return (
									<span className="px-1.5 py-0.5 bg-background shadow-minimal rounded-[4px] text-[11px] text-foreground/70">
										{normalizePath(activity.toolInput.file_path)
											.split("/")
											.pop()}
									</span>
								);
							...
							if (Array.isArray(activity.toolInput?.changes)) {
								const firstChange = activity.toolInput.changes[0] as
									| { path?: string }
									| undefined;
								if (firstChange?.path) {
									return (
				...
				>
					<div className="flex flex-col gap-0.5">
						...
						<span className="max-w-[220px] whitespace-normal text-xs leading-tight text-muted-foreground">
							Explore an alternate direction without disrupting this
							conversation’s flow.
						</span>
					</div>
				</StyledDropdownMenuItem>
...
function clearAnnotationMarks(root: HTMLElement): void {
	const annotatedInlineCodeNodes = root.querySelectorAll<HTMLElement>(
		'code[data-ca-annotation-inline-code="true"]',
	);
...
function applyTextHighlightRange(
	...
	for (const segment of segments) {
		...
		const inlineCodeParent =
			selected.parentElement?.closest<HTMLElement>("code");
		if (inlineCodeParent) {
			inlineCodeParent.setAttribute("data-ca-annotation-inline-code", "true");
			inlineCodeParent.style.backgroundColor = annotationColorToCss(
...
export function ResponseCard({
	...
	const [selectionMenuTransitionConfig, setSelectionMenuTransitionConfig] =
		useState<IslandTransitionConfig>(buildAnnotationChipEntryTransition());
	...
	const isTargetInsideAnnotationIsland = useCallback(
	...
	const activeAnnotation = useMemo(() => {
		...
		return (
			(annotations ?? []).find(
				(annotation) => annotation.id === activeAnnotationDetail.annotationId,
			) ?? null
		);
	...
	const saveFollowUp = useCallback(
		async (
			...
			if (activeAnnotationDetail) {
				...
				const existingOtherBodies = activeAnnotation.body.filter(
					(body) => body.type !== "highlight" && body.type !== "note",
				);
				const nextBody: AnnotationV1["body"] = [
					{ type: "highlight" },
					...(normalizedNote.length > 0
						? [{ type: "note", text: normalizedNote, format: "plain" } as const]
						: []),
					...existingOtherBodies,
				];
	...
	const handleSubmitAndSendFollowUp = useCallback(
		(note: string) => {
	...
	const handleOpenAnnotationDetail = useCallback(
		(
			...
			openFromAnnotation(
				{ annotationId, index, anchorX, anchorY },
				noteText,
				mode,
			);
		},
	...
	useEffect(() => {
		...
		const fallbackAnchor = {
			...
			y: contentRect
				? contentRect.top + 20
				: Math.max(24, window.innerHeight * 0.2),
		};
	...
	const showSelectionMenuFromCurrentSelection = useCallback(() => {
		...
		requestAnimationFrame(() => {
			...
			const hasRecentPointer = Boolean(
				pointer && Date.now() - pointer.ts <= SELECTION_POINTER_MAX_AGE_MS,
			);
			...
			if (rects.length > 0) {
				if (pointerY != null) {
					const rowCandidates = rects.filter(
						(rect) => pointerY >= rect.top && pointerY <= rect.bottom,
					);

					if (rowCandidates.length > 0) {
						if (pointerX != null) {
							const xContaining = rowCandidates.filter(
								(rect) => pointerX >= rect.left && pointerX <= rect.right,
							);
							if (xContaining.length > 0) {
								anchorRect = xContaining.reduce((best, rect) =>
									rect.width > best.width ? rect : best,
								);
							} else {
								anchorRect = rowCandidates.reduce((best, rect) => {
									const bestDistance = Math.min(
										Math.abs(pointerX - best.left),
										Math.abs(pointerX - best.right),
									);
						...
						anchorRect = rects.reduce((best, rect) => {
							const bestDistance = Math.abs(
								(best.top + best.bottom) / 2 - pointerY,
							);
							const rectDistance = Math.abs(
								(rect.top + rect.bottom) / 2 - pointerY,
							);
							return rectDistance < bestDistance ? rect : best;
						});
					}
	...
	const handleTextSelection = useCallback(
		(event: React.MouseEvent<HTMLDivElement>) => {
			...
			if (event.shiftKey) {
				...
				const blockElement = targetElement?.closest<HTMLElement>(
					"[data-ca-block-path]",
				);
				if (blockElement) {
					const blockPath =
						blockElement.getAttribute("data-ca-block-path") || "";
					const blockType =
						blockElement.getAttribute("data-ca-block-type") || "paragraph";
					const blockId =
						blockElement.getAttribute("data-ca-block-id") || undefined;

					if (blockPath) {
						const alreadyExists = (annotations ?? []).some((annotation) => {
							const blockSelector = annotation.target.selectors.find(
								(s) => s.type === "block",
							) as
								| Extract<
	...
	const selectionMenu = allowAnnotationIsland ? (
		<AnnotationIslandMenu
			...
			onDelete={
				activeAnnotationDetail ? handleDeleteActiveAnnotation : undefined
			}
	...
	const annotationOverlayLayer = (
		<AnnotationOverlayLayer
			...
			getTooltipText={(annotation) =>
				formatAnnotationFollowUpTooltipText(annotation)
			}
	...
	function TurnCard({
		...
		useEffect(() => {
			if (
				...
				activitiesContainerRef.current
			) {
									...
									<div
										...
										className={cn(
											...
											sortedActivities.length >
												...
												"rounded-r-md overflow-y-auto scrollbar-hover py-1.5",
										)}
										style={{
											maxHeight:
												sortedActivities.length >
				...
				{planActivities.map((planActivity, index) => (
					...
					>
						<ResponseCard
							...
							isLastResponse={
								isLastResponse && index === planActivities.length - 1
							}
							...
							onBranch={
								onBranch
									? (options?: { newPanel?: boolean }) =>
											onBranch(
												planActivity.messageId ?? planActivity.id,
												options,
											)
									: undefined
							}
						...
						{response && !isBuffering && (
							...
							>
								<ResponseCard
									...
									onOpenUrl={onOpenUrl}
									onPopOut={
										onPopOut ? () => onPopOut(response.text) : undefined
									}
									...
									compactMode={compactMode}
									onBranch={
										onBranch && response.messageId
											? (options?: { newPanel?: boolean }) =>
													onBranch(response.messageId!, options)
											: undefined
									}
									sendMessageKey={sendMessageKey}
									hasActiveFollowUpAnnotations={hasActiveFollowUpAnnotations}
