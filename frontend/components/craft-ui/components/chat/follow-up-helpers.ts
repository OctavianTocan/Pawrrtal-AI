...
export function extractAnnotationSelectedText(
	...
	const positionSelector = annotation.target.selectors.find(
		(
			...
			AnnotationV1["target"]["selectors"][number],
			{ type: "text-position" }
		> => selector.type === "text-position",
	);
	if (positionSelector) {
		const start = Math.max(
			0,
			Math.min(positionSelector.start, messageContent.length),
		);
		const end = Math.max(
			start,
			Math.min(positionSelector.end, messageContent.length),
		);
