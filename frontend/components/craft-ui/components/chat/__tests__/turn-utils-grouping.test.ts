...
describe("groupActivitiesByParent", () => {
	...
	describe("multiple Tasks with children", () => {
		it("groups each Task with its own children", () => {
			...
			const child1a = createChildActivity(task1.toolUseId!, {
				toolName: "Read",
			});
			const child1b = createChildActivity(task1.toolUseId!, {
				toolName: "Grep",
			});
			const child2a = createChildActivity(task2.toolUseId!, {
				toolName: "Write",
			});
	...
	describe("mixed orphans and Task groups", () => {
		it("preserves orphan activities alongside Task groups in chronological order", () => {
			...
			const child = createChildActivity(task.toolUseId!, {
				toolName: "Grep",
				timestamp: 2500,
			});
	...
	describe("TaskOutput data attachment", () => {
		...
		it("hides TaskOutput activities from result", () => {
			...
			const taskOutput = createTaskOutputActivity("xyz789", {
				durationMs: 1000,
			});
			...
			expect(
				result.some(
					(item) =>
						...
						(item as ActivityItem).toolName === "TaskOutput",
				),
			).toBe(false);
		});
	});
	...
	describe("edge cases", () => {
		...
		it("shows children with missing parents as orphan activities at root level", () => {
			...
			const orphanChild = createChildActivity("non-existent-parent", {
				toolName: "Read",
			});
	...
	describe("stateless invariants (post-refactor)", () => {
		...
		it("restored session with nested Task hierarchy produces correct groups", () => {
			...
			const outerTask = createTaskActivity("Explore codebase", {
				timestamp: 1000,
			});
...
describe("computeLastChildSet", () => {
	...
	it("identifies last child for single parent", () => {
		...
		const child3 = createChildActivity(parent.toolUseId!, {
			toolName: "Write",
		});
	...
	it("identifies last child for multiple parents", () => {
		...
		const child1a = createChildActivity(parent1.toolUseId!, {
			toolName: "Read",
		});
		const child1b = createChildActivity(parent1.toolUseId!, {
			toolName: "Grep",
		});
		const child2a = createChildActivity(parent2.toolUseId!, {
			toolName: "Write",
		});
	...
	it("handles single child (is both first and last)", () => {
		resetCounters();
		const parent = createActivity({ toolName: "Task" });
		const onlyChild = createChildActivity(parent.toolUseId!, {
			toolName: "Read",
		});
