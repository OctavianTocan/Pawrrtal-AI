...
import { type PlatformActions, PlatformProvider } from "../../context";
...
export function SessionViewer({
	...
	const handleExpandedActivityGroupsChange = useCallback(
		(groups: Set<string>) => {
	...
	const handleOpenActivityDetails = useCallback(
	...
	return (
		<PlatformProvider actions={platformActions}>
			<div className={cn("flex flex-col h-full", className)}>
				{/* Header */}
				{header && <div className="shrink-0 border-b">{header}</div>}
					...
					<div className="h-full overflow-y-auto">
						...
						>
							{turns.map((turn, index) => {
								...
								if (turn.type === "system") {
									const msgType =
										turn.message.role === "error"
											? "error"
											: turn.message.role === "warning"
												? "warning"
												: turn.message.role === "info"
													? "info"
													: "system";
									return (
										<SystemMessage
									...
									return (
										<TurnCard
											...
											onExpandedChange={(expanded) =>
												handleExpandedChange(assistantUiKey, expanded)
											}
											...
											onOpenMultiFileDiff={
												platformActions.onOpenMultiFileDiff
													? () =>
															platformActions.onOpenMultiFileDiff!(
																session.id,
																turn.turnId,
											...
											annotationInteractionMode={
												mode === "readonly" ? "tooltip-only" : "interactive"
											}
										/>
									);
								}
