...
import {
	type ExternalOpenAnnotationRequest,
	useAnnotationInteractionController,
} from "../annotations/use-annotation-interaction-controller";
...
export function AnnotatableMarkdownDocument({
	...
	const [selectionMenuTransitionConfig, setSelectionMenuTransitionConfig] =
		React.useState<IslandTransitionConfig>(
			buildAnnotationChipEntryTransition(),
		);
	const [annotationOverlay, setAnnotationOverlay] = React.useState<{
	...
	const isTargetInsideAnnotationIsland = React.useCallback(
	...
	const handleSubmitFollowUp = React.useCallback(
		(note: string) => {
	...
	useAnnotationIslandEvents({
		enabled:
			hasAnnotationInteraction(interactionState) && isSelectionMenuVisible,
		openedAtRef: selectionMenuOpenedAtRef,
