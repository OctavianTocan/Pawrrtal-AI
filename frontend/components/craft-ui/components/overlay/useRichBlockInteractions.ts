...
import {
	RICH_BLOCK_DEFAULTS,
	type RichBlockInteractionOptions,
} from "./rich-block-interaction-spec";
...
export function useRichBlockInteractions({
	...
	useEffect(() => {
		const handleMouseMove = (e: MouseEvent) => {
			...
			setTranslate({
				x:
					translateAtDragStartRef.current.x +
					(e.clientX - dragStartRef.current.x),
				y:
					translateAtDragStartRef.current.y +
	...
	useEffect(() => {
		...
		const handleWheel = (e: WheelEvent) => {
			...
			const sensitivity = e.ctrlKey
				? wheelSensitivity.trackpadPinch
				: wheelSensitivity.mouse;
			const factor = 2 ** (-e.deltaY * sensitivity);
