...
export function Group({
	...
	const context = useMemo(
		() => ({
			...
			togglePanelDisabled: (panelId: string, disabled: boolean) => {
				const inMemoryValues = inMemoryValuesRef.current;
				const panel = inMemoryValues.panels.find((current) => current.id === panelId);
				if (panel) {
	...
	useIsomorphicLayoutEffect(() => {
		...
		if (stableProps.defaultLayout !== undefined) {
			if (Object.keys(stableProps.defaultLayout).length === inMemoryValues.panels.length) {
		...
		const { defaultLayoutDeferred, derivedPanelConstraints, layout } = getMountedGroupState(
			group.id,
			true
		);
		...
		const removeChangeEventListener = subscribeToMountedGroup(id, (event) => {
			...
			derivedPanelConstraints.forEach((constraints) => {
				if (constraints.collapsible) {
					...
					if (prevLayout) {
						const currentSize = layout[constraints.panelId];
						const previousSize = prevLayout[constraints.panelId];
