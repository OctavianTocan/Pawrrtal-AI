...
export function Separator({
	...
	useIsomorphicLayoutEffect(() => {
		...
		if (element !== null) {
			...
			const removeInteractionStateChangeListener = subscribeToInteractionState((event) => {
				setDragState(
					event.next.state !== 'inactive' &&
						event.next.hitRegions.some((hitRegion) => hitRegion.separator === separator)
						? event.next.state
						: 'inactive'
				);
			});

			const removeMountedGroupsChangeListener = subscribeToMountedGroup(groupId, (event) => {
