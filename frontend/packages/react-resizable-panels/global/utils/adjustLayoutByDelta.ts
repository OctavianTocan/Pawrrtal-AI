...
export function adjustLayoutByDelta({
...

	{
		...
		const pivotPanelConstraints = panelConstraintsArray[pivotIndex];
		assert(pivotPanelConstraints, `Panel constraints not found for index ${pivotIndex}`);

		const unsafeSize = prevSize + deltaApplied;
		const safeSize = validatePanelSize({
			overrideDisabledPanels,
			panelConstraints: pivotPanelConstraints,
			prevSize,
			size: unsafeSize,
		});
