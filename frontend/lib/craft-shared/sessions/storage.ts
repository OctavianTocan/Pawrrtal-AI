...
import { getWorkspaceSessionsPath } from '../workspaces/storage';
import { generateUniqueSessionId } from './slug-generator';
import { toPortablePath, expandPath } from '../utils/paths';
import { sanitizeSessionId } from './validation';
...
import type { Plan } from '../agent/plan-types';
import { validateSessionStatus } from '../statuses/validation';
import { debug } from '../utils/debug';
import { getStatusCategory } from '../statuses/storage';
import { readSessionHeader, readSessionJsonl } from './jsonl';
...
function generatePlanFileName(plan: Plan, plansDir: string): string {
  const name = plan.title || plan.context?.substring(0, 50) || 'untitled';
