...
import { resolveAutomationsConfigPath, generateShortId } from './resolve-config-path';
import { compactAutomationHistorySync } from './history-store';
import { createLogger } from '../utils/debug';
import { WorkspaceEventBus, type EventPayloadMap } from './event-bus';
import { PromptHandler, EventLogHandler, WebhookHandler, type AutomationsConfigProvider } from './handlers/index';
import type { AutomationsConfig, AutomationEvent, AutomationMatcher, PendingPrompt, WebhookActionResult, AppEvent, AgentEvent, SdkAutomationCallbackMatcher, SdkAutomationInput } from './types';
import { validateAutomationsConfig } from './validation';
import { matcherMatchesSdk } from './utils';
import { SchedulerService, type SchedulerTickPayload } from '../scheduler/scheduler-service';

const log = createLogger('automation-system');

// Re-export SessionMetadataSnapshot from types (single source of truth)
export type { SessionMetadataSnapshot } from './types';
import type { SessionMetadataSnapshot } from './types';
