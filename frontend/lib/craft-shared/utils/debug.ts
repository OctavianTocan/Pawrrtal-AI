...
function getElectronLog(): { info?: (message: string) => void } | null {
  ...
  try {
    ...
    // Use an indirect loader so browser bundlers don't try to resolve it.
    const dynamicRequire = Function(
      'return typeof require !== "undefined" ? require : null',
    )() as ((id: string) => unknown) | null;
    const loaded = dynamicRequire?.(['electron-log', 'main'].join('/'));
    electronLog = (loaded as { default?: unknown } | null)?.default ?? loaded ?? null;
  } catch {
    electronLog = null;
  }
  return (electronLog as { info?: (message: string) => void } | null) ?? null;
