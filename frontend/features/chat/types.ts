...
/**
 * Inline image generated by the `generate_image` tool via Codex OAuth.
 *
 * Emitted as a sibling of the matching `tool_result` event. The `b64` field
 * is a base64-encoded PNG read from the workspace by the backend — the
 * frontend renders it inline without a separate HTTP round-trip.
 */
export interface ChatImagePayload {
	/** Stable id derived from the originating tool_use_id, e.g. `img_tc-0`. */
	id: string;
	/** Base64-encoded PNG bytes. */
	b64: string;
	/** MIME type — always `"image/png"` for gpt-image-2 output. */
	mime_type: string;
	/** Workspace-relative path where the PNG was saved, e.g. `generated_images/...`. */
	path: string;
	/** Originating tool_use_id — correlates with the `tool_use` / `tool_result` pair. */
	tool_use_id: string;
}

/** Sibling event emitted whenever the agent's `generate_image` tool succeeds. */
