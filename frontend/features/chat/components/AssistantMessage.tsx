...
interface AssistantMessageProps {
	...
	/** Inline images generated by the `generate_image` tool. */
	generatedImages?: ChatImagePayload[];
}
				...
				{generatedImages && generatedImages.length > 0 ? (
					<div className="mt-3 flex flex-col gap-3">
						{generatedImages.map((img) => (
							<GeneratedImageCard image={img} key={img.id} />
						))}
					</div>
				) : null}
...
/**
 * Renders a single inline PNG generated by the `generate_image` tool.
 *
 * The image is embedded as a base64 data URL so no additional HTTP request
 * is needed. The card mirrors the ArtifactCard shape: rounded border,
 * overflow-hidden, and a small caption below the image.
 */
function GeneratedImageCard({ image }: { image: ChatImagePayload }): ReactNode {
	const src = `data:${image.mime_type};base64,${image.b64}`;
	const filename = image.path.split('/').pop() ?? 'generated_image.png';

	return (
		<div className="rounded-lg overflow-hidden border border-border bg-muted/30">
			<img alt="AI-generated artwork" className="w-full h-auto block" src={src} />
			<div className="px-3 py-2 text-xs text-muted-foreground truncate">{filename}</div>
		</div>
	);
}
